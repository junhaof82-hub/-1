...
class MacroService:
    def _market_snapshot(self) -> dict:
        ...
        try:
            ...
            logger.warning('yfinance unavailable for macro proxies: %s', e)
        ...
        for name, ticker in MARKET_PROXIES.items():
            try:
                h = yf.Ticker(ticker).history(period='2mo', interval='1d', auto_adjust=False)
                ...
                logger.warning('Macro proxy failed %s: %s', ticker, e)
                out[name] = np.nan; out[f'{name}_5d_return'] = np.nan; out[f'{name}_20d_return'] = np.nan
        return out
